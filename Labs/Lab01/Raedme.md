...
проверим таблицу мак адресов на S2

![](11.png)

Мы видим мак адрес от сетевой карты PC-B `2cfd.a15e.49e7` на порту 8, и мак адреса от интерфейса S1 `2401.c7d2.2881` и мак адрес PC-A `58d5.6e3f.5c3e` оба адреса приходят на интерфейс `fa0/1`

По врефиксу `2cfd.a1` мы видим что от PC-B мак принадлежит устройству ASUS, `2401.c7` - Cisco, `58d5.6e` - D-Link. В целом если не задокументированы мак адреса устройств, то в небольшой сети ещё можно определить какому устройству принадлежит мак методом отключения этих устройств от сети. Чем больше и сложнее сеть, тем тяжелее будет определить мак для каждого устройства.

-Шаг 3 Очистим таблицу МАС-адресов коммутатора S2 и снова отобразим таблицу МАС-адресов

Вводим `clear mac address-table dynamic ` и быстро вводим `show mac address-table`

![](12.png)

В таблице видим только мак от коммутатора S1. Новых адресов не появилось.
